...
def cluster_models(model_select_index_list, alarm_record_list, output_val_res_list, n_components_int,parameter_set_dic):
    """
    ...
    for time_step_t, each_index_combination_list in enumerate(model_select_index_list):
        ...
        # print('each_index_combination_list:', each_index_combination_list)
        # print('cluster_num:', len(each_index_combination_list))
        # print('n_components_int:', n_components_int)
        # print('n_samples', output_val_res_list[time_step_t][:, np.array(model_select_index_list[time_step_t]) + 1].T)

        '''由于sklearn的版本问题,0.19.1版本没有问题但是0.21.*的gmm.fit无法对(1,*)shape的数据进行实验，所以采用if语句强行进行判断，手动复制'''
        if((output_val_res_list[time_step_t][:, np.array(each_index_combination_list) + 1].T).shape[0]==1):
            labels = np.array([0])
        else:
            gmm.fit(output_val_res_list[time_step_t][:, np.array(each_index_combination_list) + 1].T)
            # gmm.fit(output_val_res_list[time_step_t][:, np.array(model_select_index_list[time_step_t]) + 1].T)
            labels = gmm.predict(output_val_res_list[time_step_t][:, np.array(model_select_index_list[time_step_t]) + 1].T)

        #查看进行selection的数据的size
        # print('selection object shape: ',
        #       (output_val_res_list[time_step_t][:, np.array(each_index_combination_list) + 1].T).shape)
